...
def greatRenaming(roadRegister):
    _size = len(roadRegister)
    results = [[roadRegister[j][i] for i in range(_size)] for j in range(_size)]
    for row in range(_size):
        for col in range(row, _size):
            if row == 0:
                results[0][col] = roadRegister[-1][col - 1]
                results[col][0] = roadRegister[-1][col - 1]      
            results[row][col] = roadRegister[row-1][col-1]
            results[col][row] = roadRegister[row-1][col-1]
    return results