...
def volleyballPositions(formation, k):
    ...
    cols, rows = 3, 4
    result = [["empty" for c in range(cols)] for r in range(rows)]
    for row in range(rows):
        for col in range(cols):
            if formation[row][col] != "empty":
                new_index = (positions.index((row, col)) + k) % len(positions)
                new_position = positions[new_index]
                result[new_position[0]][new_position[1]] = formation[row][col]
    return result
