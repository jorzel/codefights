...
def serverFarm(jobs, servers):
    jobs = sorted([(job, i) for i, job in enumerate(jobs)], key=lambda x: (-x[0], x[1]))
    servers = [[0, i, []] for i in range(servers)]
    for job, index in jobs:
        servers = sorted(servers)
        servers[0][0] += job
        servers[0][2].append(index)
    return [p[2] for p in sorted(servers, key=lambda x: x[1])]
