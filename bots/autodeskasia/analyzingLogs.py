...
Each .log file consists of several lines, where each line represents one log entry. Each log has the following format:
...
You want to find the most common ERROR by analyzing the log files. To do this, you would like to create a report about the number of ERRORs each source file has. The result should be sorted by the number of errors in descending order. In case two source files have the same number of errors, the file with the lexicographically smallest name should go first.
...
logs_dir = '/root/devops/logs'
files = [f for f in os.listdir(logs_dir) if f.endswith('.log')]
sources = defaultdict(int)
for fname in files:
    with open(f"{logs_dir}/{fname}", "rt") as f:
        for line in f:
            splited_line = line.split('|')
            if len(splited_line) == 6 and splited_line[1] == 'ERROR':
                sources[splited_line[2]] +=1
